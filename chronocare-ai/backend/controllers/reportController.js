  ...
  try {
    ...
    if (req.file) {
      ...
      fileType = req.file.mimetype.includes("pdf") ? "pdf" : 
                 ...
                 (req.file.mimetype.includes("text") || req.file.mimetype === "text/plain") ? "text" : "doc");
      ...
      if (fileType === "pdf") {
        try {
          ...
          if (typeof pdfParse === 'function') {
            // Traditional pdf-parse (hubgit)
            pdfData = await pdfParse(dataBuffer);
            extractedText = pdfData.text;
          } else if (pdfParse && pdfParse.PDFParse) {
            // Modern pdf-parse (Mehmet Kozan)
            const pdf = new pdfParse.PDFParse({ data: dataBuffer });
            pdfData = await pdf.getText();
            extractedText = pdfData.text;
          } else {
            console.error("PDF Parse error: No valid pdfParse function or class found.");
          }
        } catch (pdfErr) {
...
export const getAllRoutedReports = async (req, res) => {
  try {
    ...
    const reports = await Report.find({})
      .sort({ createdAt: -1 })
      .populate("userId", "name email phone emergencyContact"); // Include contact details
    return res.json({ reports });
  } catch (err) {
...
export const reviewReport = async (req, res) => {
  try {
    ...
    // Create or Update a timeline event for the patient
    await TimelineEvent.findOneAndUpdate(
      { reportId: report._id, type: "doctor_note" },
      {
