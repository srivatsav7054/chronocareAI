          ...
          <div className="flex flex-col h-full">
                ...
                <div className="flex flex-col gap-1 text-sm text-gray-500 mb-2">
                  <div className="flex items-center gap-3">
                    <span className="flex items-center gap-1 text-gray-800 font-bold">
                      <User className="w-4 h-4" /> {selectedReport.userId?.name}
                    </span>
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    <span className="bg-gray-100 px-2 py-0.5 rounded text-gray-600">
                      Contact: {selectedReport.userId?.phone || "N/A"}
                    </span>
                    <span className="bg-red-50 px-2 py-0.5 rounded text-red-600">
                      Emergency: {selectedReport.userId?.emergencyContact || "N/A"}
                    </span>
                  </div>
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex-1 space-y-6">
                <div>
                  <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Source Clinical Text</h3>
                  <div className="bg-gray-50 text-gray-700 p-4 rounded-xl text-sm border border-gray-100 max-h-60 overflow-y-auto whitespace-pre-wrap">
                    {selectedReport.extractedText || "No clinical text extracted."}
                  </div>
                </div>
              ...
              <div className="flex-1 flex flex-col bg-gray-50 rounded-2xl p-6 border border-gray-100 shadow-inner">
                ...
                <textarea
                  ...
                  className="flex-1 w-full border border-gray-200 bg-white rounded-xl p-5 text-sm text-gray-700 focus:outline-none focus:ring-4 focus:ring-indigo-100 focus:border-indigo-300 transition-all resize-none mb-4 shadow-sm"
                ></textarea>
                
                <button
                  onClick={handleReviewSubmit}
                  disabled={submitting}
                  className={`w-full py-4 ${selectedReport.status === "completed" ? "bg-emerald-600 hover:bg-emerald-700 shadow-emerald-100" : "bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200"} text-white rounded-2xl font-black text-sm flex items-center justify-center gap-3 shadow-lg transition-all hover:-translate-y-0.5 active:translate-y-0`}
                >
                  {submitting ? (
                    ...
                    <>
                      ...
                      {selectedReport.status === "completed" ? "UPDATE ASSESSMENT" : "COMPLETE REVIEW & NOTIFY"}
                    </>
                  )}
                </button>
                ...
                {selectedReport.status === "completed" && (
                  <div className="mt-4 p-3 bg-emerald-50 border border-emerald-100 rounded-xl flex items-center gap-3">
                    ...
                    <p className="text-[10px] text-emerald-800 font-bold uppercase tracking-wider">Finalized & Shared with Patient</p>
                  </div>
                )}
              </div>
